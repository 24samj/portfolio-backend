...
- Content lives in D1 (`PORTFOLIO_DB` binding in `wrangler.jsonc`), seeded from `migrations/9999_seed_portfolio_v6.sql`. Run `bun run db:local` once before `bun run dev`.
...
- Content is read-only and shipped as a seed migration. Edit `scripts/portfolio-data.json`, run `bun run build:seed`, commit the regenerated `migrations/9999_seed_portfolio_v6.sql`. See `.claude/rules/migrations.md`.
